@extends('layout')
@section('content')
    {{-- {{dd($success)}} --}}
    @include('Partials.header');

    <div class="container">
        @include('Partials.message')
    </div>
    <div class="container center my-5">
        <div class="card w-96 bg-neutral text-neutral-content">
            <div class="card-body items-center text-center">
                <h2 class="card-title">Modifier votre photo de profil</h2>
                <form action="{{ route('photo.update') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="avatar">
                        <div class="w-24 rounded">
                            @if (auth()->user()->photo === null)
                            <img src="data:image/png;base64,"  alt="">
                            @else
                            <img src="{{auth()->user()->photo}}" alt="">
                            @endif
                        </div>
                       
                    </div>
                    
                    <div class="form-group">
                        <label for="photo">Téléverser votre photo</label><br>
                        <input type="file" class="file-input w-full max-w-xs text-neutral" name="photo"/>
                        
                    </div>

                    <button type="submit" class="btn btn-outline my-3">Mettre à jour</button>
                </form>


            </div>
        </div>
    </div>
    </div>
    @include('Partials.footer');
@endsection

